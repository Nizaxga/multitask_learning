...
dataset_name = "Banking77Classification_test"
texts = [row["text"] for row in task.dataset["test"]]
labels = [row["label"] for row in task.dataset["test"]]
label_texts = [row["label_text"] for row in task.dataset["test"]]
...
assert set(label_to_texts.keys()) <= set(label_to_labeltext.keys()), ( "Mismatch in label mapping!")
...
label_records = [(dataset_id, label_text) for _, label_text in label_to_labeltext.items()]
curr.executemany(
    """
